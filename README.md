...
`claude-mv` does the `mv` **and** re-keys the history to match, across every
configured profile.
...
$HOME with two Claude profiles, runs `claude-mv` against it, and converts the
ANSI colours to SVG. Only the window and the `%` prompt line are drawn. Every
store it touched is reported, and the run closes with the tally.)
...
```sh
zsh tools/generate-readme-svg.zsh   # → assets/{move,conflict,restore}-<hash>.svg + README refs
```

It builds a hermetic sandbox — a throwaway `$HOME` holding a folder to move, a
nested project of its own, and two seeded Claude profiles (`projects/`, session
jsonl, config json, `history.jsonl`) — and runs the tool unmodified against it
